...
single-question Q\&A, the system provides case-analysis endpoints---weakness
analysis, defence-memorandum drafting with an agentic self-check, and
forensic-consistency checking---governed by an expert-derived
procedural-defence reasoning checklist paired with a grounding-safe few-shot
exemplar, and is deployed as a production FastAPI service with multi-provider
failover, big-context routing, persistent multi-turn sessions, token
streaming, automated watch-folder ingestion with hot-reload, and a documented
.NET integration contract. Finally, we report a retrieval-quality optimisation
...
Article-Lookup Rescue \sep Iterative Retrieval \sep Article-Aware Chunking \sep
BGE-M3 \sep Dataset Construction
\end{keyword}
...
\begin{enumerate}
  ...
  \item A \textbf{production-grade operational architecture}
        (Sections~\ref{sec:arch},~\ref{sec:deploy}): a multi-provider,
        multi-key failover chain with big-context routing for large case
        files, persistent compacting multi-turn sessions, token streaming,
...
ordered provider chain---Groq (\texttt{llama-3.3-70b-versatile}) $\rightarrow$
Cerebras $\rightarrow$ xAI $\rightarrow$ Google Gemini $\rightarrow$ OpenRouter
Qwen---in which each provider rotates across its own list of API keys on
HTTP~429/quota errors before the chain falls through to the next provider.
This multi-key, multi-provider rotation is what lets the service run on free
tiers despite per-minute and per-day token caps. A complementary
\emph{big-context routing} rule guards against oversized prompts: each
provider declares a maximum request size (\texttt{PROVIDER\_MAX\_PROMPT\_CHARS};
e.g.\ 26{,}000 chars for Groq, 80{,}000 for OpenRouter), and a request whose
assembled prompt exceeds a provider's budget transparently skips that provider
and is routed to a larger-context tier. This is what allows the case-analysis
endpoints to accept full case files (up to 50{,}000 characters) without
failing on the primary provider's context window.
...
\subsection{Agentic Self-Check for Defence Memoranda}\label{sec:selfcheck}

The defence-memorandum endpoint (\texttt{/defense}) applies an additional
agentic grounding pass on top of the per-answer evidence validation of
Section~\ref{sec:validate}. Because a memorandum is a long, multi-paragraph
document, a single evidence-validation gate is coarse: it flags an ungrounded
article but does not by itself repair the surrounding argument. Instead, the
draft memorandum is fed back to the LLM under a dedicated reviewer prompt
(\texttt{verify\_memo}) whose sole instruction is to make the memo
\emph{fully grounded} in the provided material---removing or correcting any
article number not present in the retrieved texts, deleting any assertion the
case facts do not support, and introducing no new citation or fact---while
preserving the four-part structure (\ar{الوقائع}, \ar{الإطار القانوني},
\ar{أوجه الدفاع}, \ar{الطلبات}). This draft\,$\rightarrow$\,verify\,$\rightarrow$\,revise
loop runs up to \texttt{MEMO\_SELF\_CHECK\_MAX\_ITERS} times (default 1) and
stops early once a pass introduces no further change. The number of revision
passes actually applied is surfaced to the client as the
\texttt{self\_check\_revisions} response field, and the whole mechanism is
toggleable via \texttt{MEMO\_SELF\_CHECK}.
...
access is restored. Table~\ref{tab:crossmodel} gives the intended reporting
format.

\begin{table}[!htb]
\centering
\small
\caption{Cross-model comparison reporting template (results pending a full
run). The grounding column applies the evidence validator of
Section~\ref{sec:validate} uniformly to all three systems.}
...
\paragraph{Configurability} Operational parameters---retrieval $k$, chunk
sizes, confidence weights and threshold, temperatures, provider chain and
per-provider request budgets, embedding and reranker selection, session
limits, and self-check iterations---are centralised in a single
\texttt{config.py} (overridable by environment variables), so behaviour can be
tuned for a deployment without code changes.
...
\textbf{Embedding model and retrieval ceiling.} The retrieval-quality
optimisation of Section~\ref{sec:retropt} (v9)---the BGE-M3 (1{,}024-dim)
embedding upgrade, article-aware chunking, and the widened reranker pool---directly
addresses what was previously this paper's single largest deferred improvement.
The reranker widening is live and article-aware chunking is implemented (applied
online to new ingests and validated on a sample); the BGE-M3 embedding rollout is
deferred to a GPU-equipped environment because, as quantified in
Section~\ref{sec:retropt}, BGE-M3 embedding on the CPU-only reference machine
(whose GPU is unusable due to a compute-capability mismatch with current PyTorch)
costs $\sim$5.5\,s/chunk and projects to multiple days for the full corpus. The
build/swap tooling and evaluation harness are in place so the rollout reduces to a
single batch run plus a \texttt{--compare} scorecard once accelerator access is
available.
